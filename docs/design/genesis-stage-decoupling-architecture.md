...
#### 任务列表
- [ ] 创建 Genesis 新表的 Alembic 迁移脚本
- [ ] 创建三张新表：`genesis_flows`、`genesis_stage_records`、`genesis_stage_sessions`
- [ ] 运行迁移并验证表结构
...
#### 任务列表
- [ ] 实现 `GenesisFlowService`（ensure_flow/get_flow/advance/complete）
- [ ] 实现 `GenesisStageService`（create_stage/add_stage_session/list_stage_sessions）
- [ ] 实现绑定校验与错误处理（scope 与 novel 归属）
- [ ] 编写服务层单元测试
...
#### 任务列表
- [ ] 创建 Genesis 只读查询 API（按阶段列出会话，按小说获取流程）
- [ ] 创建 Genesis 写入 API（创建流程、创建阶段、绑定会话）
- [ ] 更新 API 文档
...
- [ ] 绑定校验测试（错误 scope_type/scope_id 拒绝）
- [ ] 多会话场景测试（一个阶段允许多个会话）
...
| 投入项目 | 预估工时 | 实际价值 |
|---------|---------|---------|
| 数据库迁移 | 16小时 | 新增业务表，保持现有系统稳定 |
| 服务层实现 | 24小时 | 业务解耦，支持多会话场景 |
| API接口开发 | 16小时 | 增量API，不影响现有接口 |
| 集成测试 | 8小时 | 质量保证，验证解耦效果 |
| **总计** | **64小时** | **低风险业务解耦** |

**ROI评估**: 投入 1.6人周，低侵入性设计降低风险，预期在 1个月内见效，长期ROI > 200%
...
这是一个稳健且高效的解耦方案，将为 InfiniteScribe 的创世功能提供强大支撑。