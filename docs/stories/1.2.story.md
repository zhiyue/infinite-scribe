...
- [ ] Task 1: 准备开发服务器部署环境 (AC: 1)
  - [ ] Subtask 1.1: SSH连接到开发服务器 (zhiyue@192.168.2.201)
  - [ ] Subtask 1.2: 在服务器上创建项目目录结构
  - [ ] Subtask 1.3: 将本地项目文件同步到服务器（使用rsync或git）
  - [ ] Subtask 1.4: 确保服务器上的docker和docker-compose版本兼容

- [ ] Task 2: 更新docker-compose.yml配置所有核心服务 (AC: 1, 2)
  - [ ] Subtask 2.1: 确认现有docker-compose.yml中的服务配置正确（PostgreSQL, Redis, Neo4j, Milvus, Minio, Kafka, Zookeeper已存在）
  - [ ] Subtask 2.2: 添加或更新服务的健康检查配置
  - [ ] Subtask 2.3: 确保所有服务使用相同的网络配置
  - [ ] Subtask 2.4: 配置服务间的依赖关系（如Milvus依赖etcd和minio）

- [ ] Task 3: 创建数据库初始化脚本 (AC: 4, 5)
  - [ ] Subtask 3.1: 创建 `infrastructure/docker/init/postgres/01-init-schema.sql` 包含所有表定义和索引 [Source: architecture/数据库模式.md#PostgreSQL]
  - [ ] Subtask 3.2: 创建 `infrastructure/docker/init/postgres/02-init-functions.sql` 包含触发器函数
  - [ ] Subtask 3.3: 创建 `infrastructure/docker/init/minio/create-buckets.sh` 脚本自动创建novels bucket
  - [ ] Subtask 3.4: 更新docker-compose.yml挂载初始化脚本到容器

- [ ] Task 4: 配置环境变量和端口映射 (AC: 3, 4, 5, 7)
  - [ ] Subtask 4.1: 验证.env.example中包含所有必需的环境变量（包括Redis密码）
  - [ ] Subtask 4.2: 在服务器上创建.env文件并设置安全的生产值
  - [ ] Subtask 4.3: 在docker-compose.yml中正确引用环境变量
  - [ ] Subtask 4.4: 配置端口映射考虑服务器防火墙规则
  - [ ] Subtask 4.5: 为Redis配置内存限制和过期策略参数
...
- [ ] Task 8: 配置Prefect工作流编排服务 (AC: 2, 6)
  - [ ] Subtask 8.1: 在docker-compose.yml中添加Prefect Server服务配置
  - [ ] Subtask 8.2: 配置Prefect使用PostgreSQL作为后端数据库（创建prefect数据库）
  - [ ] Subtask 8.3: 设置Prefect API URL和端口映射（4200）
  - [ ] Subtask 8.4: 添加Prefect环境变量到.env.example（PREFECT_API_URL, PREFECT_API_KEY）
  - [ ] Subtask 8.5: 创建Prefect初始化脚本确保数据库schema正确创建
