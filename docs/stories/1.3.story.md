...
### File Locations
- API网关应用位置: `apps/api-gateway/` [Source: architecture/source-tree.md#apps]
- FastAPI应用代码: `apps/api-gateway/app/` [Source: architecture/source-tree.md#api-gateway]
- API路由模块: `apps/api-gateway/app/api/` [Source: architecture/source-tree.md#api-gateway]
- 核心配置: `apps/api-gateway/app/core/` [Source: architecture/source-tree.md#api-gateway]
- 服务层: `apps/api-gateway/app/services/` [Source: architecture/source-tree.md#api-gateway]
- 主入口: `apps/api-gateway/app/main.py` [Source: architecture/source-tree.md#api-gateway]
- 依赖文件: `apps/api-gateway/pyproject.toml` [Source: architecture/source-tree.md#api-gateway]
- Docker配置: `apps/api-gateway/Dockerfile` [Source: architecture/source-tree.md#api-gateway]
...
- 测试文件放在 `apps/api-gateway/tests/` 目录 [Source: architecture/source-tree.md#api-gateway]
...
- [ ] Task 1: 创建API网关基础项目结构 (AC: 1)
  - [ ] Subtask 1.1: 在 `apps/api-gateway` 创建FastAPI项目目录结构
  - [ ] Subtask 1.2: 创建/更新根目录 `pyproject.toml` 添加所有服务依赖（如果不存在）
  - [ ] Subtask 1.3: 在根目录运行 `uv venv` 和 `uv sync --dev` 初始化环境
  - [ ] Subtask 1.4: 创建 `app/main.py` 主入口文件和基础FastAPI应用
  - [ ] Subtask 1.5: 设置 `app/core/config.py` 配置管理（环境变量读取）

- [ ] Task 2: 实现数据库连接服务 (AC: 2)
  - [ ] Subtask 2.1: 在 `app/services/` 创建PostgreSQL连接服务 (`postgres_service.py`)
  - [ ] Subtask 2.2: 在 `app/services/` 创建Neo4j连接服务 (`neo4j_service.py`)
  - [ ] Subtask 2.3: 实现异步数据库连接池管理和连接验证函数
  - [ ] Subtask 2.4: 在启动时测试数据库连接并记录连接状态

- [ ] Task 3: 实现健康检查端点 (AC: 3)
  - [ ] Subtask 3.1: 在 `app/api/v1/` 创建health路由模块 (`health.py`)
...
- [ ] Task 4: 配置Docker容器化 (AC: 4, 5)
  - [ ] Subtask 4.1: 创建 `apps/api-gateway/Dockerfile` 配置Python 3.11基础镜像
  - [ ] Subtask 4.2: 配置Docker使用根目录的 `pyproject.toml` 进行多阶段构建
