...
4. **Message Relay服务关键点**：
   - 使用定时任务（如每5秒）轮询event_outbox
   - 批量处理以提高效率
   - 实现优雅关闭，确保不丢失正在处理的消息
...
- [ ] Pytest Integration Test: location: `apps/backend/tests/integration/test_message_relay.py`
  - 使用testcontainers启动PostgreSQL和Kafka
  - 验证端到端的命令处理和事件发布流程
