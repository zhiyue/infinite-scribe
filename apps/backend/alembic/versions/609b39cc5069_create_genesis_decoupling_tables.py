...
def upgrade() -> None:
    ...
    # Handle existing enum types before creating tables
    connection = op.get_bind()

    # Check if genesisstatus enum exists and add PAUSED if missing
    result = connection.execute(sa.text("""
        SELECT EXISTS(
            SELECT 1 FROM pg_type WHERE typname = 'genesisstatus'
        )
    """)).scalar()
    ...
    if result:
        ...
        paused_exists = connection.execute(sa.text("""
            SELECT EXISTS(
                ...
                WHERE pg_type.typname = 'genesisstatus' AND enumlabel = 'PAUSED'
            )
        """)).scalar()

        if not paused_exists:
            # Add PAUSED to existing enum
            connection.execute(sa.text("ALTER TYPE genesisstatus ADD VALUE 'PAUSED'"))
    else:
        # Create the enum if it doesn't exist
        sa.Enum('IN_PROGRESS', 'COMPLETED', 'ABANDONED', 'PAUSED', name='genesisstatus').create(connection)

    # Check if genesisstage enum has all required values
    stage_result = connection.execute(sa.text("""
        SELECT EXISTS(
            SELECT 1 FROM pg_type WHERE typname = 'genesisstage'
        )
    """)).scalar()

    if not stage_result:
        # Create genesisstage enum if it doesn't exist (shouldn't happen but safety check)
        sa.Enum('INITIAL_PROMPT', 'WORLDVIEW', 'CHARACTERS', 'PLOT_OUTLINE', 'FINISHED', name='genesisstage').create(connection)

    # Create new enum types that shouldn't exist yet
    stagestatus_exists = connection.execute(sa.text("""
        SELECT EXISTS(
            SELECT 1 FROM pg_type WHERE typname = 'stagestatus'
        )
    """)).scalar()

    if not stagestatus_exists:
        sa.Enum('RUNNING', 'COMPLETED', 'FAILED', 'PAUSED', name='stagestatus').create(connection)

    stagesessionstatus_exists = connection.execute(sa.text("""
        SELECT EXISTS(
            SELECT 1 FROM pg_type WHERE typname = 'stagesessionstatus'
        )
    """)).scalar()

    if not stagesessionstatus_exists:
        sa.Enum('ACTIVE', 'ARCHIVED', 'CLOSED', name='stagesessionstatus').create(connection)
    ...
    sa.Column('status', postgresql.ENUM('IN_PROGRESS', 'COMPLETED', 'ABANDONED', 'PAUSED', name='genesisstatus', create_type=False), nullable=False, comment='流程状态：IN_PROGRESS/COMPLETED/ABANDONED/PAUSED'),
    sa.Column('current_stage', postgresql.ENUM('INITIAL_PROMPT', 'WORLDVIEW', 'CHARACTERS', 'PLOT_OUTLINE', 'FINISHED', name='genesisstage', create_type=False), nullable=True, comment='当前所在阶段：INITIAL_PROMPT/WORLDVIEW/CHARACTERS/PLOT_OUTLINE/FINISHED'),
    ...
    sa.Column('stage', postgresql.ENUM('INITIAL_PROMPT', 'WORLDVIEW', 'CHARACTERS', 'PLOT_OUTLINE', 'FINISHED', name='genesisstage', create_type=False), nullable=False, comment='阶段类型：INITIAL_PROMPT/WORLDVIEW/CHARACTERS/PLOT_OUTLINE'),
    sa.Column('status', postgresql.ENUM('RUNNING', 'COMPLETED', 'FAILED', 'PAUSED', name='stagestatus', create_type=False), nullable=False, comment='阶段状态：RUNNING/COMPLETED/FAILED/PAUSED'),
    ...
    sa.Column('status', postgresql.ENUM('ACTIVE', 'ARCHIVED', 'CLOSED', name='stagesessionstatus', create_type=False), nullable=False, comment='关联状态：ACTIVE/ARCHIVED/CLOSED'),
