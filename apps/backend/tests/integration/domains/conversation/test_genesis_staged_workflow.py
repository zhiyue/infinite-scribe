...
class TestGenesisStagedWorkflow:
    ...
    async def test_complete_genesis_workflow_progression(self, genesis_fixture):
        ...
        stage_update = await genesis_fixture.update_session_stage(
            ...
            state={"current_stage": GenesisStage.THEME_CONCEPT.value, "stage_0_completed": True}
        )
        ...
        stage_update = await genesis_fixture.update_session_stage(
            stage=GenesisStage.CHARACTER,
            state={"current_stage": GenesisStage.CHARACTER.value, "stage_2_completed": True}
        )
        assert stage_update["success"]
        ...
        stage_update = await genesis_fixture.update_session_stage(
            stage=GenesisStage.PLOT,
            state={"current_stage": GenesisStage.PLOT.value, "stage_3_completed": True}
        )
        assert stage_update["success"]
        ...
        # Check final stage
        final_stage = session_data.get("stage") if hasattr(session_data, "get") else getattr(session_data, "stage", None)
        assert final_stage == GenesisStage.PLOT.value
    ...
    async def test_stage_validation_and_transitions(self, genesis_fixture):
        ...
        async with genesis_fixture.session_factory() as db:
            ...
            invalid_transition = await conversation_service.update_session(
                ...
                stage=GenesisStage.CHARACTER.value  # Skip stages 1 and 2
            )
        ...
        stages = [
            GenesisStage.THEME_CONCEPT,
            GenesisStage.WORLDVIEW,
            GenesisStage.CHARACTER,
            GenesisStage.PLOT
        ]
        ...
        for stage in stages:
            ...
            await genesis_fixture.create_conversation_round(
                ...
                input_data={"prompt": f"进入{stage.value}阶段", "stage": stage.value}
            )
    ...
    async def test_stage_completion_criteria(self, genesis_fixture):
        ...
        creative_seed_rounds = [
            {
                "role": DialogueRole.USER,
                "input_data": {"prompt": "我想创作科幻小说", "intent": "start_creation"}
            },
            {
            ...
            },
            {
                ...
                "input_data": {
                    "selected_concept": 2,
                    "lock_decision": True,
                    "completion_criteria": "concept_locked"
                }
            }
        ]
        ...
        theme_development_rounds = [
            {
                "role": DialogueRole.USER,
                "input_data": {
                    "prompt": "基于AI觉醒的概念，探讨主题",
                    "builds_on_stage_0": True
                }
            },
            {
                "role": DialogueRole.ASSISTANT,
        ...
        completion_markers = [
            r for r in genesis_fixture.conversation_history
            if "completion_criteria" in r["input_data"]
        ]
    ...
    async def test_stage_branching_and_alternative_paths(self, genesis_fixture):
        ...
        alternative_markers = [
            r for r in conversation_history
    ...
    async def test_workflow_interruption_and_resumption(self, genesis_fixture):
        ...
        # Start Stage 1
        stage_update = await genesis_fixture.update_session_stage(GenesisStage.THEME_CONCEPT)
        assert stage_update["success"]
        ...
        stage_1_partial = await genesis_fixture.create_conversation_round(
            ...
            input_data={
                ...
                "interruption_point": "before_completion"
            }
        )
        ...
        theme_completion = await resumed_fixture.create_conversation_round(
            role=DialogueRole.ASSISTANT,
            input_data={
                "theme_development": "基于之前的讨论，完善主题",
                "acknowledges_interruption": True
            }
        )
    ...
    async def test_workflow_quality_gates(self, genesis_fixture):
        ...
        quality_gates = {
            ...
            GenesisStage.THEME_CONCEPT: {
                "min_rounds": 3, 
                "required_elements": ["theme_statement", "emotional_core"],
        ...
        for stage, criteria in quality_gates.items():
            ...
            stage_rounds = [
                r for r in genesis_fixture.conversation_history 
                if r["input_data"].get("stage") == stage.value
            ]
