...
async def test_enqueue_command_persists_domain_event_and_outbox(pg_session):
    ...
    create_res = await conversation_service.create_session(
        pg_session, user_id=user.id, scope_type=ScopeType.GENESIS, scope_id=str(novel.id)
    )
    print(f"DEBUG: create_session result: {create_res}")
    ...
    dom_event = await pg_session.scalar(
    ...
    out = await pg_session.scalar(select(EventOutbox).where(EventOutbox.id == dom_event.event_id))
...
@pytest.mark.integration
@pytest.mark.requires_docker
async def test_create_round_persists_round_created_and_outbox(pg_session):
    ...
    dom_evt = await pg_session.scalar(
        select(DomainEvent).where(
    ...
    out = await pg_session.scalar(select(EventOutbox).where(EventOutbox.id == dom_evt.event_id))
