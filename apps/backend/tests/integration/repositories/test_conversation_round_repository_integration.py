...
from src.schemas.novel.dialogue import SessionStatus, DialogueRole
...
class TestConversationRoundRepositoryIntegration:
    ...
    async def test_create_and_persist_round(
        self, 
        postgres_test_session: AsyncSession, 
        ...
        found_round = await new_session.get(ConversationRound, round_obj.id)
    ...
    async def test_find_by_session_and_path_database_query(
        ...
        # Act
        found_round = await repository.find_by_session_and_path(
            test_session.id, 
            round_path
        )
    ...
    async def test_find_by_correlation_id_database_query(
        ...
        # Act
        found_round = await repository.find_by_correlation_id(
            test_session.id, 
            correlation_id
        )
    ...
    async def test_list_by_session_role_filtering(
        ...
        # Act
        user_rounds = await repository.list_by_session(
            test_session.id, 
            role=DialogueRole.USER
        )
        assistant_rounds = await repository.list_by_session(
            test_session.id, 
    ...
    async def test_cascade_delete_with_session(
        ...
        # Verify rounds are also deleted
        new_session = AsyncSession(postgres_test_session.bind)
        found_round1 = await new_session.get(ConversationRound, round1.id)
        found_round2 = await new_session.get(ConversationRound, round2.id)
        await new_session.close()
    ...
    async def test_timestamp_consistency_and_timezone(
        ...
        assert before_creation <= found_round.updated_at <= after_creation
        assert found_round.created_at.tzinfo is not None
        assert found_round.updated_at.tzinfo is not None
