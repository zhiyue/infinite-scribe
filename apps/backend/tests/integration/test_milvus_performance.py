...
class TestMilvusPerformance:
    ...
    async def test_batch_insert_performance(self, milvus_service):
        ...
        try:
            ...
            if 10 in performance_results and 100 in performance_results:
                ...
                ), f"Large batch should be more efficient: {large_batch_rate:.2f} vs {small_batch_rate:.2f} items/sec"
    ...
    async def test_vector_search_performance_optimization(self, milvus_service):
        ...
        try:
            ...
            for config_name, result in performance_results.items():
                assert (
                    result["avg_search_time"] < 0.4
                ), f"Search {config_name} should complete < 400ms, took {result['avg_search_time']*1000:.2f}ms"
                assert result["success"] is True, f"Search {config_name} should return results"

            # Performance comparison: verify all configs are reasonably fast for small dataset
            # For small datasets, ef parameter differences may not be significant
            max_time = max(result["avg_search_time"] for result in performance_results.values())
            assert (
                max_time < 0.05
            ), f"All search configurations should be fast for small dataset, max time: {max_time*1000:.2f}ms"
    ...
    async def test_concurrent_operations_performance(self, milvus_service):
        ...
        try:
            ...
            async def perform_search(search_id: int):
                ...
                results = await embedding_manager.search_similar_content_optimized(
                    collection_name, query_embedding=query_embedding, novel_id="concurrent_test", limit=5
                )

                search_time = time.time() - start_time
                return {"search_id": search_id, "search_time": search_time, "results_count": len(results)}

            # Run concurrent searches
            num_concurrent_searches = 5
            start_time = time.time()

            concurrent_results = await asyncio.gather(*[perform_search(i) for i in range(num_concurrent_searches)])
            ...
            assert (
                avg_search_time < 0.4
            ), f"Average concurrent search time should be < 400ms, got {avg_search_time*1000:.2f}ms"
    ...
    async def test_memory_usage_optimization(self, milvus_service):
