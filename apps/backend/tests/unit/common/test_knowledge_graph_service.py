...
async def test_knowledge_graph_service_enhanced_methods():
    ...
    with patch("src.common.services.knowledge_graph.knowledge_graph_service.create_neo4j_session") as mock_session, \
         ...
         patch("src.common.services.knowledge_graph.knowledge_graph_service.Neo4jRelationshipManager") as MockRelationshipManager:
        ...
        result = await kg.create_world_rule_node(
            ...
            novel_id="novel-1",
            dimension="physics",
            rule="Gravity works normally"
        )
        assert result is True
        
        result = await kg.create_location_node(
            location_id="loc-1",
            novel_id="novel-1",
            name="Castle",
            x=10.0,
        ...
        result = await kg.create_character_relationship(
            character1_id="char-1",
            character2_id="char-2",
            strength=8,
            rel_type="friend"
        )
        assert result is True
        ...
        result = await kg.create_world_rule_conflict(
            rule1_id="rule-1",
            rule2_id="rule-2",
            severity="major"
        )
...
async def test_knowledge_graph_service_query_builder():
    ...
    with patch("src.common.services.knowledge_graph.knowledge_graph_service.GraphQueryBuilder") as MockQueryBuilder:
        kg = KnowledgeGraphService()
        query_builder = kg.create_query_builder()
