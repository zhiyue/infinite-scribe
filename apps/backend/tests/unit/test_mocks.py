...
class MockEmailServiceClass:
    ...
    async def send_verification_email(self, email: str, token: str):
        ...
        self.sent_emails.append({"type": "verification", "email": email, "token": token})
        return {"success": True}

    async def send_welcome_email(self, email: str, name: str):
        """Mock send welcome email."""
        self.sent_emails.append({"type": "welcome", "email": email, "name": name})
        return {"success": True}

    async def send_password_reset_email(self, email: str, token: str):
        """Mock send password reset email."""
        self.sent_emails.append({"type": "password_reset", "email": email, "token": token})
        return {"success": True}
