...
class CommandInboxResponse(BaseSchema, TimestampMixin):
    """命令收件箱响应模型"""

    id: UUID = Field(..., description="命令唯一标识符")
    ...
    status: CommandStatus = Field(..., description="命令处理状态")
    error_message: str | None = Field(None, description="处理失败时的错误信息")
    retry_count: int = Field(..., description="重试次数")


class AsyncTaskResponse(BaseSchema, TimestampMixin):
    """异步任务响应模型"""

    id: UUID = Field(..., description="任务唯一标识符")
    task_type: str = Field(..., description="任务类型")
    triggered_by_command_id: UUID | None = Field(None, description="触发此任务的命令ID")
    status: TaskStatus = Field(..., description="任务执行状态")
    progress: Decimal = Field(..., description="任务进度(0.00 - 100.00)")
...
class EventOutboxResponse(BaseSchema):
    """事件发件箱响应模型"""

    id: UUID = Field(..., description="消息唯一标识符")
    ...
    status: OutboxStatus = Field(..., description="消息发送状态")
    retry_count: int = Field(..., description="重试次数")
    max_retries: int = Field(..., description="最大重试次数")
...
class FlowResumeHandleResponse(BaseSchema, TimestampMixin):
    """工作流恢复句柄响应模型"""

    id: UUID = Field(..., description="句柄唯一标识符")
    ...
    status: HandleStatus = Field(..., description="回调句柄的状态")
    resume_payload: dict[str, Any] | None = Field(None, description="用于存储提前到达的恢复数据")
    timeout_seconds: int | None = Field(None, description="句柄超时时间(秒)")
