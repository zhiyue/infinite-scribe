...
class ServiceStatusItem(BaseModel):
    ...
    status: str = Field(description="Service status: running/starting/stopped/error")
...
class LauncherStatusResponse(BaseModel):
    ...
    timestamp: str = Field(default_factory=lambda: datetime.now(UTC).isoformat() + "Z", description="Query timestamp")
