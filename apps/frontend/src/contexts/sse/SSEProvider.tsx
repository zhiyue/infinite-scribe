...
import { saveSSEState, getSSEState, clearSSEState, shouldResumeFromLastEvent } from '@/utils/sseStorage'
...
export const SSEProvider: React.FC<SSEProviderProps> = ({
  ...
  const jitter = (current: number, min: number, max: number) => {
    ...
    const r = Math.random() * (SSE_CONNECTION_CONFIG.JITTER_RANGE.MAX - SSE_CONNECTION_CONFIG.JITTER_RANGE.MIN) + SSE_CONNECTION_CONFIG.JITTER_RANGE.MIN;
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    retryDelayRef.current = Math.min(retryDelayRef.current * SSE_CONNECTION_CONFIG.RETRY_MULTIPLIER, maxRetry);
  ...
  useEffect(() => {
    const onVis = () => {
      if (document.hidden) {
        ...
        retryDelayRef.current = Math.max(retryDelayRef.current, minRetry * SSE_CONNECTION_CONFIG.HIDDEN_DELAY_MULTIPLIER);
  ...
  // 订阅事件
  const subscribe = useCallback((event: string, cb: Listener) => {
