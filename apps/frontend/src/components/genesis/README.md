...
**最新更新 (2025-01-11)**:
- **欢迎消息优化**: 改进欢迎消息显示逻辑，始终显示友好的引导界面
- **无 commandId SSE 订阅**: 即使没有 commandId 也启用 SSE 事件订阅，以便接收 Genesis 系统事件
- **增强事件处理**: 优化 SSE 事件处理逻辑，支持更广泛的事件类型
- **智能命令推断**: 改进命令 ID 推断算法，支持从多个数据源获取命令 ID
- **状态持久化**: 增强页面刷新后的状态恢复机制
- **性能优化**: 减少不必要的重渲染和 API 调用
- **错误处理**: 完善错误分类和用户友好的错误提示
- **事件时间线**: 集成 useCommandEvents 提供完整的事件时间线追踪
