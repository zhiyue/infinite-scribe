...
test.describe('密码重置流程', () => {
  ...
  test('请求密码重置', async ({ page }) => {
    ...
    await page.waitForSelector(
      '.success-message, h1:has-text("Check your email"), h1:has-text("检查你的邮箱")',
      { timeout: 5000 },
    )
  ...
  test('请求密码重置 - 不存在的邮箱', async ({ page }) => {
    ...
    const message = await page
      .locator('.success-message, .text-green-500, .error-message')
      .textContent()
  ...
  test('重置密码 - 有效令牌', async ({ page, request }) => {
    ...
    // 等待成功状态
    await page.waitForSelector(
      '.success-message, h1:has-text("Password Reset Successful"), h1:has-text("密码重置成功")',
      { timeout: 5000 },
    )
  ...
  test('重置密码 - 无效令牌', async ({ page }) => {
    ...
    expect(errorMessage).toMatch(
      /无效.*令牌|invalid.*token|Invalid or expired|unexpected error|reset failed/i,
    )
  })
  ...
  test('新密码验证', async ({ page, request }) => {
    ...
    // 匹配实际的错误消息
    expect(errorMessage).toMatch(
      /at least 8 characters|至少.*8.*字符|must contain.*uppercase.*lowercase.*number|必须包含.*大写.*小写.*数字/i,
    )
  ...
  test('返回登录页链接', async ({ page }) => {
    ...
    // 等待链接可见后再点击
    await page.waitForSelector('[data-testid="back-to-login-link"]', {
      state: 'visible',
      timeout: 10000,
    })
