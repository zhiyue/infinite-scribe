...
export async function fillRegisterForm(
  ...
  // 如果有确认密码字段
  const confirmPasswordField = page.locator('input[name="confirmPassword"], input[name="password_confirmation"], input[type="password"]:nth-of-type(2)');
...
export async function isUserLoggedIn(page: Page): Promise<boolean> {
  // 检查是否存在登出按钮或用户菜单
  const logoutButton = page.locator('button:has-text("Logout"), button:has-text("登出"), button:has-text("Sign out")');
 ...
 * 验证密码强度提示
 */
export async function validatePasswordStrength(page: Page, expectedStrength: 'weak' | 'medium' | 'strong') {
...
export const TEST_CONFIG = {
  ...
  // 环境检查
  isMailDevEnabled: () => {
    return process.env.USE_MAILDEV !== 'false' && 
