...
export class BasePage {
      ...
      for (const selector of selectors) {
        try {
          ...
          for (const element of elements) {
            ...
            console.log(
              `[getErrorMessage] Checking selector "${selector}": "${text}", visible: ${isVisible}`,
            )
  ...
  async getSuccessMessage(): Promise<string | null> {
    try {
      const selector =
        '[data-testid="success-message"], .success-message, .text-green-500, [role="status"]'
      ...
      try {
        ...
        console.log(
          `[getSuccessMessage] Found ${successElements.length} success elements via locator`,
        )
...
export class LoginPage extends BasePage {
  ...
  async login(email: string, password: string) {
    ...
    await Promise.race([
      ...
      // 邮箱验证需求：显示验证提示
      this.page.waitForSelector('[role="alert"] .error-message', {
        timeout: 15000,
      }),
      // 等待按钮重新启用（表示请求完成）
...
export class RegisterPage extends BasePage {
  ...
  async register(username: string, email: string, password: string, confirmPassword?: string) {
    console.log(
      `[RegisterPage] Attempting registration with email: ${email}, username: ${username}`,
    )
    ...
    await Promise.race([
      // 成功：显示成功消息或页面改变
      this.page.waitForSelector('.success-message, [data-testid="success-message"]', {
        timeout: 15000,
      }),
      // 失败：显示错误消息
      this.page.waitForSelector('[role="alert"], .error-message', {
...
export class ResetPasswordPage extends BasePage {
  ...
  async resetPassword(newPassword: string, confirmPassword?: string) {
    ...
    await Promise.race([
      // 成功：显示成功消息或页面改变
      this.page.waitForSelector(
        '.success-message, [data-testid="success-message"], h1:has-text("Password Reset Successful"), h1:has-text("密码重置成功")',
        { timeout: 15000 },
      ),
      // 失败：显示错误消息
      this.page.waitForSelector('[role="alert"], .error-message', {
...
export class ChangePasswordPage extends BasePage {
  ...
  async navigateWithAuth(loginPage: any, testUser: any) {
    ...
    await this.page.waitForSelector('[data-testid="change-password-card"]', {
      timeout: 10000,
    })
  }
  ...
  async changePassword(currentPassword: string, newPassword: string, confirmPassword?: string) {
    ...
    await Promise.race([
      // 成功：显示成功消息
      this.page.waitForSelector('.success-message, [role="status"]', {
        timeout: 15000,
      }),
      // 失败：显示错误消息
      this.page.waitForSelector(
      ...
      .waitForFunction(
        () => {
          const loadingElement = document.querySelector(
            'h1:has-text("Verifying your email"), .text-2xl',
          )
...
export class DashboardPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.changePasswordLink = page.locator(
      'a:has-text("Change Password"), a:has-text("Change password"), a:has-text("修改密码")',
    )
