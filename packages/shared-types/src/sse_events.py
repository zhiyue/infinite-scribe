...
class SSEMessage(BaseModel):
    ...
    id: Optional[str] = Field(None, description="事件 ID，格式：{source}:{partition}:{offset}")
    retry: Optional[int] = Field(None, description="重连延迟（毫秒）", ge=0, le=300000)
...
class ContentUpdateEvent(BaseModel):
    ...
    summary: Optional[str] = Field(None, description="更新摘要")
...
class SSEErrorEvent(BaseModel):
    ...
    correlation_id: Optional[str] = Field(None, description="关联请求 ID")
    retry_after: Optional[int] = Field(None, description="建议重试时间（秒）")
