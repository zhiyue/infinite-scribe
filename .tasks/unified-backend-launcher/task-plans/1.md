...
| 库/模块 | 方法/类 | 用途 | 文档链接 |
|---------|--------|------|---------|
| pathlib | Path | 创建目录结构 | Python标准库 |
| toml | load/dump | 更新pyproject.toml | Python标准库 |
...
**测试先行（RED）**：
```python
# 测试文件：tests/test_launcher_structure.py
import pytest
...
**测试先行（RED）**：
```python
# 测试文件：tests/test_launcher_modules.py
...
def test_cli_entry_point_registered():
    ...
    assert "scripts" in config["project"] or "console_scripts" in config["project"]["scripts"]
...
# 3. 验证结构
tree apps/backend/src/launcher/
