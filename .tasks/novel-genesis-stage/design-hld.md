...
- 消费
  `genesis.session.events`（领域事实），按业务决策派发能力"请求事实"（`*.Requested`）到对应
  `*.tasks`
- 消费各 `*.events`
  的能力结果（`*.Created/Updated/Completed/...`），合成领域事实并写回
...
// 领域请求 → 能力任务
{
  "event_type": "Genesis.Session.Theme.Requested",    // 点式命名
  "topic": "genesis.outline.tasks"
}

// 能力结果 → 领域事实
{
  "event_type": "Chapter.Outline.Created",            // 点式命名
  "topic": "genesis.outline.events"
}

// Orchestrator 合成领域事实
{
  "event_type": "Genesis.Session.Plot.Created",       // 点式命名
